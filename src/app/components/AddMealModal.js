...
const validatePortion = (value) => {
  // Allow letters, numbers, spaces, common punctuation and Nigerian food units
  const validPattern = /^[a-zA-Z0-9\s\-\+\/\(\)\.,%½⅓¼⅔¾]+$/
  return validPattern.test(value)
}

const validateImageUrl = (url) => {
  if (!url) return true // Empty is optional
  try {
...
const validateIngredientName = (name) => {
  // Allow letters, numbers, spaces, common food characters
  const pattern = /^[a-zA-Z0-9\s\-\'\.]+$/
  return pattern.test(name)
}

const validateIngredientQuantity = (quantity) => {
  // Allow numbers, fractions, common units
