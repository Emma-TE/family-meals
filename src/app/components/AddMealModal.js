...
export default function AddMealModal({ isOpen, onClose, onMealAdded }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let finalImageUrl = 'https://images.unsplash.com/photo-1604329760661-e71dc83f8f26?w=400'
      ...
      if (error) { toast.error(error.message); setError(error.message) }
        ...
        onMealAdded(data[0])
        onClose()
        setName(''); setCategory('breakfast'); setCalories(''); setPortion(''); setPrepTime(''); setImageFile(null); setIngredients([{ name: '', quantity: '' }])
      }
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()} style={{ backgroundColor: stitchTheme.colors.surface, boxShadow: stitchTheme.shadows.xl }}>
        <div className={styles.header} style={{ backgroundColor: stitchTheme.colors.surface, borderColor: stitchTheme.colors.outlineVariant }}>
          <div>
            <div className={styles.subTitle} style={{ color: stitchTheme.colors.primary }}>Add New Recipe</div>
            <h2 className={styles.title} style={{ color: stitchTheme.colors.onSurface }}>Create a Meal</h2>
          </div>
        ...
        <div className={styles.formContent}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className={styles.formGroup}>
              <label className={styles.formLabel} style={{ color: stitchTheme.colors.onSurface }}>Meal Photo <span className={styles.helperText} style={{ color: stitchTheme.colors.onSurfaceVariant }}>(optional)</span></label>
            ...
            <div className={styles.formActions} style={{ borderColor: stitchTheme.colors.outlineVariant }}>
              ...
              <button type="submit" disabled={loading} className={styles.btnPrimary} style={{ background: loading ? '#ccc' : stitchTheme.colors.primary }}>{loading ? 'Adding...' : 'Add Meal'}</button>
            </div>
          </form>
        </div>
      </div>
